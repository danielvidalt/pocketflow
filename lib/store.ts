...
import type { IncomeSource, IncomeEntry, Expense, DebtPocket, SavingsGoal } from './types'
import { weeklyEquivalent } from './types'
interface State {
  ...
  weeklyIncome:()=>number; todayExpenses:()=>Expense[]; weekExpenses:()=>Expense[]
}
...
export const usePocketFlow = create<State>((set,get) => ({
  incomeSources:[], incomeEntries:[], expenses:[], debtPockets:[], savingsGoals:[], loading:false, exchangeRates:{},
  fetchAll: async () => {
    set({loading:true}); const db=getClient()
    const [s,e,ex,d,g] = await Promise.all([
    ...
    set({incomeSources:s.data||[],incomeEntries:e.data||[],expenses:ex.data||[],debtPockets:d.data||[],savingsGoals:g.data||[],loading:false})
  },
